...
pub fn build() {
    ...
    assert_unique_feature!(
        "arm", "aarch64", "i386", "x86_64", "mips", "ppc", "hexagon", "riscv32", "riscv64", "tricore"
    );
    ...
    // TriCore is little-endian only
    assert_unique_feature!("be", "aarch64", "i386", "x86_64", "hexagon", "riscv32", "riscv64", "tricore");
    ...
    let cpu_target = if cfg!(feature = "x86_64") {
        ...
        "hexagon".to_string()
    } else if cfg!(feature = "tricore") {
        "tricore".to_string()
    } else {
        env::var("CPU_TARGET").unwrap_or_else(|_| {
            println!(
                "cargo:warning=No architecture feature enabled or CPU_TARGET env specified for libafl_qemu, supported: arm, aarch64, hexagon, i386, mips, ppc, riscv32, riscv64, tricore, x86_64 - defaulting to x86_64"
            );
            "x86_64".to_string()
        })
    };
    ...
    println!("cargo::rustc-check-cfg=cfg(cpu_target, values(\"x86_64\", \"arm\", \"aarch64\", \"i386\", \"mips\", \"ppc\", \"hexagon\", \"riscv32\", \"riscv64\", \"tricore\"))");
